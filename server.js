...
// ------------------------- MAPA RESERVATÓRIOS - DECLARADO AQUI EM CIMA -------------------------
...
function buildDashboard(dados) {
  ...
  return {
    lastUpdate: dados.timestamp? new Date(dados.timestamp).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" }) : "--:--",
    reservatorios,
    pressoes,
    bombas,
    bombasLigadas
...
app.get("/api/dashboard", (req, res) => {
  ...
  if (!dados || Object.keys(dados).length === 0) {
    return res.json({
      ...
      kpis: { elevador_hoje: 0, lavanderia_hoje: 0, osmose_hoje: 0 }
    });
  }

  const dashboard = buildDashboard(dados);
  const consumoData = safeReadJson(CONSUMO_FILE, { historico: [] });
  dashboard.kpis = {
    elevador_hoje: consumoData.historico.reduce((s, i) => s + i.v, 0),
    lavanderia_hoje: 0,
    osmose_hoje: consumoData.historico.reduce((s, i) => s + i.v, 0)
  };
  dashboard.manutencao = getManutencao().ativo;
  dashboard.alerta_consumo = safeReadJson(ALERTA_FILE, {});
